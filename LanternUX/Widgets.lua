...
local function RefreshActiveWidgets()
    ...
    -- Label Actions
    for _, w in ipairs(pools["label_action"] or {}) do
        if (w._inUse) then
            local disabled = EvalDisabled(w);
            if (disabled) then
    ...
    -- Item Rows
    for _, w in ipairs(pools["item_row"] or {}) do
        if (w._inUse) then
            local disabled = EvalDisabled(w);
            if (disabled) then
                w._text:SetTextColor(unpack(T.disabledText));
...
-------------------------------------------------------------------------------
-- Widget: Label Action (text left, button right on same row)
-------------------------------------------------------------------------------
...
-------------------------------------------------------------------------------
-- Widget: Item Row (icon + name + remove button + item tooltip)
-------------------------------------------------------------------------------
...
local widgetFactories = {
    ...
    label_action = { create = CreateLabelAction,   setup = SetupLabelAction },
    item_row     = { create = CreateItemRow,       setup = SetupItemRow },
